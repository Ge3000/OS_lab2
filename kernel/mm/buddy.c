                                ...
                                struct page *__maybe_unused chunk)
{
        ...
        /*check if the chunk is the largest one*/
        if (chunk->order == (BUDDY_MAX_ORDER-1)){
                return chunk;
        }
        /*locate the buddy chunk */
        buddy_chunk = get_buddy_chunk(pool,chunk);

        /*check if buddy_trunk not exist,no ferther merge*/
        if (buddy_chunk == NULL)
                return chunk;

        /*if not free,not ferthur merge needed*/
        if (buddy_chunk->allocated == 1)
                return chunk;

        /*if the buddy_chunk is not free as a whole, no further merge needed*/
        if (buddy_chunk->order != chunk->order)
                return chunk;

        /*remove the buddy_chunk from the current list */
        list_del(&(buddy_chunk->node));
        pool->free_lists[buddy_chunk->order].nr_free -= 1;

        /*merge the chunk to get larger chunk*/
        buddy_chunk->order +=1;
        chunk->order +=1;
